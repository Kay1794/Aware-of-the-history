...
class ArgoDataset(Dataset):
    def __init__(self, split, config, train=True, precompute_mode=False):
        self.config = config
        self.train = train
        ...
        self.ct_valid_point = 0.
        self.total_point = 0.
        self.max_grid_dist = -1.
        self.behavior_empty = 0
    ...
    def __getitem__(self, idx):
        ...
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            new_data = dict()
            for key in ['city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph','argo_id']:
                if key in data:
                    new_data[key] = ref_copy(data[key])
         
            # add behavior scene feature 
            if self.config['behavior']:
                
                behavior_idx = idx
                behavior_data = None
                behavior_data = np.load(self.config['behavior_root']+'/'+self.file_split+'/'+str(behavior_idx)+'.pickle',allow_pickle=True)

                new_data['behavior'] = {}
                behavior_ctrs = behavior_data[:,0]
                new_data['behavior']['ctrs'] = behavior_ctrs
                new_data['behavior']['feats'] = behavior_data[:,:self.config['n_behavior']//2,:]
                    

                new_data['argo_id'] = idx
                data = new_data
...
class ArgoTestDataset(ArgoDataset):
    def __init__(self, split, config, train=False):
        ...
        self.file_split = 'test'
        self.avl = ArgoverseForecastingLoader(split2)
        self.avl.seq_list = sorted(self.avl.seq_list)
        print(len(self.avl.seq_list))
        if 'preprocess' in config and config['preprocess']:
            if train:
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.config['behavior']:
                ...
                behavior_ctrs = behavior_data[:,0]
                new_data['behavior']['ctrs'] = behavior_ctrs
                new_data['behavior']['feats'] = behavior_data-behavior_ctrs.reshape(-1,1,2)
