...
from layers import Conv1d, Res1d, Linear, LinearRes, Null,MLP,LinearReverseRes,ReverseRes1d
...
class Net(nn.Module):
	...
	def __init__(self, config):
		...
		if config['double_actornet']:
			self.actor_pred = PredNet(config)
	...
	def forward(self, data: Dict,training=True) -> Dict[str, List[Tensor]]:
		...
		if config['double_actornet']:
			out_actor = self.actor_pred(actors, actor_idcs, actor_ctrs)
		else:
			out_actor = None
		...
		#
		actors = self.behavior_fuse(actors, actor_idcs, actor_ctrs, behavior_nodes, behavior_idcs, behavior_ctrs)
		actors = self.pre_a2a(actors, actor_idcs, actor_ctrs)
...
class BehaviorNet(nn.Module):
	...
	def forward(self, behavior):
		...
		behavior_ctrs = behavior["ctrs"]
		behavior_idcs = behavior["idcs"]
		behavior = behavior['feats'].reshape(-1,config['n_behavior'])
		feat = self.input(behavior)
...
class A2A(nn.Module):
	...
	def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor],use_relu: bool=True) -> Tensor:
		for i in range(len(self.att)):
			actors = self.att[i](
				actors,
				actor_idcs,
...
class Att(nn.Module):
	...
	def forward(self, agts: Tensor, agt_idcs: List[Tensor], agt_ctrs: List[Tensor], ctx: Tensor, ctx_idcs: List[Tensor], ctx_ctrs: List[Tensor], dist_th: float,use_relu: bool=True) -> Tensor:
		res = agts
		if len(ctx) == 0:
			agts = self.agt(agts)
			agts = self.relu(agts)
...
class PECLoss(nn.Module):
	...
	def forward(self, out: Dict[str, List[Tensor]],has_preds: List[Tensor]) -> Dict[str, Union[Tensor, int]]:

		dest, gt_dest = out["dest"], out["gt_dest"]
		mean,log_var = out['mu'],out["var"]
		dest = torch.cat([x for x in dest], 0)
		gt_dest = torch.cat([x for x in gt_dest], 0)
		has_preds = torch.cat([x[:,-1] for x in has_preds], 0)
		loss_out = dict()
		zero = 0.0 * (dest.sum() + gt_dest.sum())
		loss_out["dest_loss"] = zero.clone()
		loss_out["kld"] = 0.0
		loss_out["num_dest"] = 0

		loss_out["dest_loss"] += self.reg_loss(
			dest[has_preds], gt_dest[has_preds]
		)
		loss_out["num_dest"] += has_preds.sum().item()
		loss_out["kld"] = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
